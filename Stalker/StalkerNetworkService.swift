...
class StalkerNetworkService: NSObject {
    ...
    func fetchThumbnailImage(urlString:String, completion:@escaping ((_ image:UIImage?, _ error:Error?)->())){
        
        guard let imageURL = URL(string: urlString)else {
            return
        }
        imageDownloader.downloadImage(for: URLRequest(url: imageURL), success: { (request, response, image) in
